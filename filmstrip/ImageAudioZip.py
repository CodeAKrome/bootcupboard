...
"""Create mp4 video from a list of images and sound files."""


class ImageAudioZip:
    def __init__(self, mp3_files, png_files):
        if not mp3_files or not png_files:
            raise ValueError("You must provide at least one MP3 and PNG file")
    ...
    def create_video(self, output_path="output.mp4", fps=24, transition_duration=2):
        ...
        for i, (png, mp3) in enumerate(zip(self.png_files, self.mp3_files)):
            ...
            if i > 0:
                # Add a crossfadein transition to the beginning of the current clip execpt first clip
        ...
        if clips:
            ...
            raise ValueError("No clips produced from the provided images and sounds.")
        ...
        final_video.write_videofile(
            output_path, codec="libx264", fps=fps, verbose=False, logger=None
        )
