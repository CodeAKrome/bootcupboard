...
def create_documents(data: List[Dict[str, Any]], keys_to_embed: List[str]) -> List[Dict[str, Any]]:
    documents = []
    for item in data:
        content = " ".join(str(item.get(key, "")) for key in keys_to_embed)
        doc = {"content": content, "meta": item}
...
def index_documents(document_store: Neo4jDocumentStore, documents: List[Dict[str, Any]]):
    preprocessor = PreProcessor()
    docs = preprocessor.process(documents)
    
    embedder = OllamaDocumentEmbedder(model="nomic-embed-text", url="http://localhost:11434")
    doc_embeddings = embedder.embed(docs)
    
    document_store.write_documents(docs)
    document_store.update_embeddings(retriever=EmbeddingRetriever(document_store=document_store))
...
def setup_search_pipeline(document_store: Neo4jDocumentStore) -> Pipeline:
    ...
    search_pipeline.add_node(component=text_embedder, name="TextEmbedder", inputs=["Query"])
    search_pipeline.add_node(component=retriever, name="Retriever", inputs=["TextEmbedder"])

    return search_pipeline

def search(pipeline: Pipeline, query: str) -> List[Dict[str, Any]]:
