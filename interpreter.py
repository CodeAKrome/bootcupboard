...
class Interpreter:
    def __init__(self):
        ...
        self.api_key = "litellm-api-key"
        self.auto_run = False
        self.local = False
        self.model = "gpt-4"
        self.debug_mode = False
        self.api_base = (
            "https://proxy.litellm.ai"  # Will set it to whatever OpenAI wants
        )
        self.context_window = 2000  # For local models only
        self.max_tokens = 750  # For local models only
        # Azure OpenAI
        self.use_azure = False
        self.azure_api_base = "litellm-api-key"
        self.azure_api_version = "https://proxy.litellm.ai"
    ...
    def respond(self):
        ...
        if not self.local:
            ...
            #
            #
            # prompt = (f"CodeLlama: {self.user_input}")
            print(f"RAVAGE! ->{prompt}<-")
            # prompt.replace('[/INST]', '')
            ...
            outputs = self.llama_instance.generate(inputs, max_new_tokens=750)
            response = tokenizer.decode(outputs[0])
            print(f"RRR: {response}")
